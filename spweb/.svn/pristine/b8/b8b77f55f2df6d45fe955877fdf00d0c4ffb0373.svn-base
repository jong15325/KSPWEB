						...
						<c:if test="${sessionScope.userInfoSession.usr_name != null}">
							<div class="ibox-content" style="padding-bottom: 0px;">
								<div style="text-align: end;">
                            		<form id="searchFrm" name="searchFrm" method="get" action="freeBoardInsert.do">
                            			<button class="btn btn-default dim" type="submit" id="freeBoardInsert" value="글쓰기">
                        			...
                        			<table class="footable table table-stripped toggle-arrow-tiny" data-page-size="15">
                                		...
                                		<c:forEach items="${aList}" var="FreeBoardDTO">
                                			<tr>
                                    			...
                                    			<td>
                                    				<div>
                                      					<a style="width:100%;font-size:16px;display:block;color:black;" href="${content}">${FreeBoardDTO.article_title}</a>
                                   						<div>
                                   							<div id="writer-area">
                            	...
                            	<nav>
									<ul class="pagination">
										<c:if test="${freeBoardPageDto.startPage > 1}">
											<li class="page_item">
												<a class="page-link" aria-label="Previous" href="freeBoardSearch.do?currentPage=${freeBoardPageDto.startPage-freeBoardPageDto.blockPage}&option=${map.option}&keyword=${map.keyword}">
										...
										<c:forEach var="i" begin="${freeBoardPageDto.startPage}" end="${freeBoardPageDto.endPage}">
											<c:url var="currPage" value="freeBoardSearch.do">
											...
											<c:if test="${freeBoardPageDto.totalPage > freeBoardPageDto.endPage}">
												<li class="page_item">
													<a class="page-link" aria-label="Next" href="freeBoardSearch.do?currentPage=${freeBoardPageDto.startPage+freeBoardPageDto.blockPage}&option=${map.option}&keyword=${map.keyword}">
