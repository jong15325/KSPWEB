...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#noticeBoardInsert').on("click", function(){
			$.ajax({
				success : function(){
					location.href = "noticeBoardInsert.do";
				},
		...
		$('#noticeBoardCheckDelete').bind('click', function() {
...
#noticeBoardInsert:hover, #noticeBoardCheckDelete:hover{
	 	...
	 	<%@ include file="../../template/header.jsp"%>
            ...
            <div class="wrapper wrapper-content animated fadeInRight ecommerce">
       			<div class="ibox-content m-b-sm border-bottom">
					<form action="noticeBoardSearch.do">
							...
							<div class="input-group-prepend">
								...
								<span id="search_concept">제목</span>
									...
									<ul class="dropdown-menu" x-placement="bottom-start" style="position: absolute; top: 35px; left: 0px; will-change: top, left;">
										<li><a href="#noticeBoardSearchTitle">제목</a></li>
										<li><a href="#noticeBoardSearchWriter">글쓴이</a></li>
										<li><a href="#noticeBoardSearchContent">내용</a></li>
									</ul>
							</div>
							<input type="hidden" class="form-control" name="option" value="noticeBoardSearchTitle" id="search_param">
                	...
                	<div class="col-lg-12">
							...
							<div class="ibox-content" style="padding-bottom: 0px;">
								<div style="text-align: end;">
									<c:if test="${sessionScope.userInfoSession.usr_admin >= 6}">
                            			<button class="btn btn-default dim" type="submit" id="noticeBoardCheckDelete" value="글삭제">
                            				<i class="fa fa-times"></i>
                            			</button>
                    					<button class="btn btn-default dim" id="noticeBoardInsert" value="글쓰기">
                        	...
                        	<c:choose>
                        		<c:when test="${aList != null}">
                        			<table class="footable table table-stripped toggle-arrow-tiny" data-page-size="15">
                                		...
                                		<tbody>
                                		<c:forEach items="${aList}" var="NoticeBoardDTO">
                                			<c:url var="content" value="noticeBoardContentRead.do">
												<c:param name="currentPage" value="${noticeBoardPageDto.currentPage}" />
												<c:param name="article_no" value="${NoticeBoardDTO.article_no}" />
												<c:if test="${sessionScope.userInfoSession.usr_name != null}">
                                			...
                                			<tr class="boardWarp" onclick="document.location.href='${content}'" style="cursor:pointer;">
												...
												<fmt:formatDate value="${NoticeBoardDTO.article_regdate}" pattern="yyyy-MM-dd" var="reg_date" />
                                    			<c:if test="${sessionScope.userInfoSession.usr_admin >= 6}">
                                    				<td style="vertical-align: middle;" onclick='event.cancelBubble=true;'>
                                    						<input id="check-${NoticeBoardDTO.article_no}" class="magic-checkbox" type="checkbox" name="listCheckBox" value="${NoticeBoardDTO.article_no}"/>
                                    				<label for="check-${NoticeBoardDTO.article_no}"></label>                     				
                                    				</td>
                                    			</c:if>
                                    			<td>
                                    				<div>
                                      					<div style="width:100%;font-size:16px;display:block;color:black;">${NoticeBoardDTO.article_title}</div>
                                   						<div>
                                   							<div id="writer-area">
                                   								<i id= "fa-pencil" class="fa fa-pencil"> </i>
                                   									<span>${NoticeBoardDTO.article_writer}</span>
                                   							</div>
                                   							<div id="day-area">
																<c:choose>
																	<c:when test="${reg_date == today}">
																		<i id= "fa-calendar" class="fa fa-clock-o"> </i>
																		<span><fmt:formatDate value="${NoticeBoardDTO.article_regdate}" pattern="HH:mm" /></span>
																	</c:when>
																	<c:otherwise>
																		<i id= "fa-calendar" class="fa fa-calendar"> </i>
																		<span><fmt:formatDate value="${NoticeBoardDTO.article_regdate}" pattern="MM-dd" /></span>
																	</c:otherwise>
																</c:choose>
                                   							</div>
														</div>
                                    			...
                                    			<div id="reply-area">
                                    				...
                                    				${NoticeBoardDTO.reply_count}
                                    			</div>
                                    			<div id="reply-area">
                                    				<i id="fa-eye" class="fa fa-eye"></i>
                                    				${NoticeBoardDTO.article_viewcnt}
                                    			</div>
                                    			</td>
                                			</tr>
                                		</c:forEach>
						...
						<div class="ibox-content">
                            <div align="center">
                            	<nav>
									<ul class="pagination">
										<c:if test="${noticeBoardPageDto.startPage > 1}">
											<li class="page_item">
												<a class="page-link" aria-label="Previous" href="noticeBoardMain.do?currentPage=${noticeBoardPageDto.startPage-noticeBoardPageDto.blockPage}">
													<span><i style="font-weight:bold" class="fa fa-angle-left"></i></span>
												</a>
											</li>
										</c:if>
										<c:forEach var="i" begin="${noticeBoardPageDto.startPage}" end="${noticeBoardPageDto.endPage}">
											<c:url var="currPage" value="noticeBoardMain.do">
												<c:param name="currentPage" value="${i}" />
											</c:url>
											<c:choose>
												<c:when test="${ i >=1 }">
													<c:choose>
														<c:when test="${i == noticeBoardPageDto.currentPage}">
											...
											<c:if test="${noticeBoardPageDto.totalPage > noticeBoardPageDto.endPage}">
												<li class="page_item">
													<a class="page-link" aria-label="Next" href="noticeBoardMain.do?currentPage=${noticeBoardPageDto.startPage+noticeBoardPageDto.blockPage}">
