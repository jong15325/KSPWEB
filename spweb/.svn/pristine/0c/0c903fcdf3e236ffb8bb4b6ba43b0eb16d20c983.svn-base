...
<script type="text/javascript">
	...
	function messageViewParam(value){
		...
		$('#messageBoxFrm').attr('action', 'myMessageView.do');
               							...
               							<c:forEach items="${aList}" var="MessageDTO">
											...
											<fmt:formatDate value="${MessageDTO.send_date}" pattern="yyyy-MM-dd" var="reg_date" />
               								<c:choose>
		               							...
		               							<c:otherwise>
		               								<tr class="unread" id="messageBox-${MessageDTO.num}" onclick="messageViewParam(${MessageDTO.num})" style="cursor:pointer;">
														<td class="check-mail" style="vertical-align: middle;width: 1%;" onclick='event.cancelBubble=true;'>
		                    								<input id="check-${MessageDTO.num}" class="magic-checkbox" type="checkbox" name="listCheckBox" value="${MessageDTO.num}"/>
		                                    				<label for="check-${MessageDTO.num}"></label> 
		                    							</td>
                            		...
                            		<nav>
										<ul class="pagination">
											<c:if test="${messagePageDto.startPage > 1}">
												<li class="page_item">
													<a class="page-link" aria-label="Previous" href="myMessageSearch.do?currentPage=${messagePageDto.startPage-messagePageDto.blockPage}&option=${map.option}&keyword=${map.keyword}">
											...
											<c:forEach var="i" begin="${messagePageDto.startPage}" end="${messagePageDto.endPage}">
												<c:url var="currPage" value="myMessageSearch.do">
											...
											<c:if test="${messagePageDto.totalPage > messagePageDto.endPage}">
												<li class="page_item">
													<a class="page-link" aria-label="Next" href="myMessageSearch.do?currentPage=${messagePageDto.startPage+messagePageDto.blockPage}&option=${map.option}&keyword=${map.keyword}">
