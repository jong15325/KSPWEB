                    						...
                    						<a class="row" href="${content}">
                    							<div class="col-4">
                        							<div class="text-center">
                            							...
                            							<c:choose>
                            								<c:when test="${UserDTO.usr_online_state == 1}">
                            									<div class="m-t-xs font-bold" style="color:steelblue;">Online</div>
                            								</c:when>
                            								<c:otherwise>
                            									<div class="m-t-xs font-bold">Offline</div>
                            								</c:otherwise>
                            							</c:choose>
                            							<div class="m-t-xs font-bold"><i class="fa fa-thumbs-up"></i> ${UserDTO.usr_like_count}</div>
                        							</div>
                    							</div>
                    							<div class="col-8">
                        							<h3><strong>${UserDTO.usr_name}</strong></h3>
                        								<p style="font-size:11px;margin-bottom: 0px;"><strong>최근 접속일</strong></p>
                        								<p style="font-size:11px;"><fmt:formatDate value="${UserDTO.time}" pattern="MM월 dd일  HH시 mm분"/></p>
                        								<div>아래 정보</div>
                    							</div>
                        					</a>
                						</div>
            		...
            		<nav>
            			<ul class="pagination">
            				<c:if test="${userPageDto.startPage > 1}">
            					<li class="page_item">
            						<a class="page-link" aria-label="Previous" href="userList.do?currentPage=${userPageDto.startPage-userPageDto.blockPage}">
            				...
            				<c:forEach var="i" begin="${userPageDto.startPage}" end="${userPageDto.endPage}">
            					<c:url var="currPage" value="userList.do">
							...
							<c:if test="${userPageDto.totalPage > userPageDto.endPage}">
								<li class="page_item">
									<a class="page-link" aria-label="Next" href="userList.do?currentPage=${userPageDto.startPage+userPageDto.blockPage}">
