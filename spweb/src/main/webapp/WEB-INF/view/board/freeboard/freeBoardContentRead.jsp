...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#freeBoardMain').bind('click', listRun);
		$('#freeBoardUpdate').bind('click', updateRun);
		$('#freeBoardDelete').bind('click', deleteRun);
		...
		$(document).on("click",".replyBtnDiv button", function() {
			switch($(this).text())	{
				...
				case "수정":
					...
					$.ajax({
						method : "post",
						url : "freeReplyUpdateContentView.do",
						data : form_data,
						success : function(data){
		...
		function listRun() {
			$('#readFrm').attr('action', "freeBoardMain.do").submit();
		}

		function updateRun() {
			$('#readFrm').attr('action', "freeBoardUpdate.do").submit();
		}
		...
		function deleteRun() {
			...
			.then((willDelete) => {
				if (willDelete) {
					$('#readFrm').attr('action', "freeBoardDeleteProcess.do").submit();
				} else {
					swal("삭제를 취소하였습니다");
				}
			});
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                        ...
                                        <form name="readFrm" id="readFrm" method="get">
                                        	...
                                        	<button id = "freeBoardMain" class="btn btn-primary btn-xs" type="button">목록</button>
											<c:if test="${sessionScope.userInfoSession.usr_name == contentView.article_writer}">
                                       			<button id = "freeBoardUpdate" class="btn btn-white btn-xs" type="button">수정</button>
												<button id = "freeBoardDelete" class="btn btn-white btn-xs" type="button">삭제</button>
                                       		</c:if>
										</form>
                                </div>
                                <div class="col-md-6">
