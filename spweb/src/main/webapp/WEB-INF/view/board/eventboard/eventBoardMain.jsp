...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#eventBoardInsert').on("click", function(){
			$.ajax({
				success : function(){
					location.href = "eventBoardInsert.do";
				},
		...
		$('#eventBoardCheckDelete').bind('click', function() {
...
#eventBoardInsert:hover, #eventBoardCheckDelete:hover{
							...
							<div class="ibox-content" style="padding-bottom: 0px;">
								<div style="text-align: end;">
									<c:if test="${sessionScope.userInfoSession.usr_admin >= 6}">
                            			<button class="btn btn-default dim" type="submit" id="eventBoardCheckDelete" value="글삭제">
                            				<i class="fa fa-times"></i>
                            			</button>
                            			<button class="btn btn-default dim" id="eventBoardInsert" value="글쓰기">
                                			...
                                			<tr class="boardWarp" onclick="document.location.href='${content}'" style="cursor:pointer;">
                                    			...
                                    			<c:if test="${sessionScope.userInfoSession.usr_admin >= 6}">
                                    				<td style="vertical-align: middle;" onclick='event.cancelBubble=true;'>
                                    						<input id="check-${EventBoardDTO.article_no}" class="magic-checkbox" type="checkbox" name="listCheckBox" value="${EventBoardDTO.article_no}"/>
                                    				<label for="check-${EventBoardDTO.article_no}"></label>                     				
                                    				</td>
                                    			</c:if>
                                    			<td>
                                    				<div>
                                      					<div style="width:100%;font-size:16px;display:block;color:black;">${EventBoardDTO.article_title}</div>
                                   						<div>
                                   							<div id="writer-area">
                            	...
                            	<nav>
									<ul class="pagination">
										<c:if test="${eventBoardPageDto.startPage > 1}">
											<li class="page_item">
												<a class="page-link" aria-label="Previous" href="eventBoardMain.do?currentPage=${eventBoardPageDto.startPage-eventBoardPageDto.blockPage}">
										...
										<c:forEach var="i" begin="${eventBoardPageDto.startPage}" end="${eventBoardPageDto.endPage}">
											<c:url var="currPage" value="eventBoardMain.do">
											...
											<c:if test="${eventBoardPageDto.totalPage > eventBoardPageDto.endPage}">
												<li class="page_item">
													<a class="page-link" aria-label="Next" href="eventBoardMain.do?currentPage=${eventBoardPageDto.startPage+eventBoardPageDto.blockPage}">
