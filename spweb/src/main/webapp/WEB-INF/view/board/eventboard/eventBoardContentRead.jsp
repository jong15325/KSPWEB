...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#eventBoardMain').bind('click', listRun);
		$('#eventBoardUpdate').bind('click', updateRun);
		$('#eventBoardDelete').bind('click', deleteRun);
		...
		$('#replyAddSuccess').on("click", function(){
			...
			$.ajax({
				method : "post",
				dataType : "json",
				url : "eventReplyInsertProcess.do",
				data : form_data,
			...
			switch($(this).text())	{
				case "삭제":
					$.ajax({
						method : "post",
						dataType : "json",
						url : "eventReplyDeleteProcess.do",
				...
				case "수정":
					...
					$.ajax({
						method : "post",
						url : "eventReplyUpdateContentView.do",
						data : form_data,
						success : function(data){
		...
		$('#replyUpdate').on('click', function(){
			...
			$.ajax({
				method : "post",
				dataType : "json",
				url : "eventReplyUpdateProcess.do",
				data : form_data,
				success : function(data){
					location.reload(true);
				},
		...
		function listRun() {
			$('#readFrm').attr('action', "eventBoardMain.do").submit();
		}

		function updateRun() {
			$('#readFrm').attr('action', "eventBoardUpdate.do").submit();
		}
		...
		function deleteRun() {
			...
			.then((willDelete) => {
				if (willDelete) {
					$('#readFrm').attr('action', "eventBoardDeleteProcess.do").submit();
				} else {
					swal("삭제를 취소하였습니다");
				}
			});
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                        ...
                                        <form name="readFrm" id="readFrm" method="get">
                                        	...
                                        	<button id = "eventBoardMain" class="btn btn-primary btn-xs" type="button">목록</button>
											<c:if test="${sessionScope.userInfoSession.usr_name == contentView.article_writer}">
                                       			<button id = "eventBoardUpdate" class="btn btn-white btn-xs" type="button">수정</button>
												<button id = "eventBoardDelete" class="btn btn-white btn-xs" type="button">삭제</button>
                                       		</c:if>
										</form>
                                </div>
                                <div class="col-md-6">
